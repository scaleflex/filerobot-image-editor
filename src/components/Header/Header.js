...
export default class extends Component {
  render() {
    ...
    const cancelBtnClosingFn = () =>
      onClose(ON_CLOSE_STATUSES.TOOLBAR_CANCEL_BTN_CLICKED);
    ...
    return (
      <HeaderWrapper className="spzheaderwrapper">
        <HeaderTop className="spzheadertop">
          <Title noCapitalStrs={noCapitalStrs} className="spzheadertitle">
            {t[`toolbar.${filteredName}`] || t[`header.image_editor_title`]}
          </Title>{" "}
          <ButtonsWrapper className="spzheaderbtnwrapper">
            {replaceCloseWithBackButton && (
              <BackButtonWrapper className="spzbackbtnwrapper">
                <svg
                  width="14"
                  height="13"
                  viewBox="0 0 14 13"
