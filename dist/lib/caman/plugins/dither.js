...
Caman.Plugin.register("dither", function (algo) {
  ...
  var curAlgo = algos[algo]; // Make sure we know to subtract a pixel, so as to contribute to the down-left of the current pixel

  var matrixWidthAdj = Math.floor(curAlgo.matrix[0].length / 2);

  var ind = function ind(x, y) {
    return (y * width + x) * 4;
  ...
  for (y = 0, end2 = height, asc2 = 0 <= end2; asc2 ? y < end2 : y > end2; asc2 ? y++ : y--) {
    ...
    for (x = 0, end3 = width, asc3 = 0 <= end3; asc3 ? x < end3 : x > end3; asc3 ? x++ : x--) {
      ...
      var r = pixels[ind(x, y)];
      var g = pixels[ind(x, y) + 1];
      var b = pixels[ind(x, y) + 2]; // The formula used to convert color to grayscale is a more accurate representation
  ...
  for (y = 0, end4 = height, asc4 = 0 <= end4; asc4 ? y < end4 : y > end4; asc4 ? y++ : y--) {
    ...
    for (x = 0, end5 = width, asc5 = 0 <= end5; asc5 ? x < end5 : x > end5; asc5 ? x++ : x--) {
      ...
      var asc6, end6;
      var newVal = output[x][y] > 128 ? 255 : 0; // The difference between the current value and the full B/W value will determine
      ...
      var clampedVal = Math.max(0, Math.min(output[x][y], 255));
      var error = (clampedVal - newVal) / curAlgo.divisor;
      output[x][y] = newVal;
      ...
      for (i = 0, end6 = curAlgo.matrix.length, asc6 = 0 <= end6; asc6 ? i < end6 : i > end6; asc6 ? i++ : i--) {
        ...
        for (j = 0, end7 = curAlgo.matrix[i].length, asc7 = 0 <= end7; asc7 ? j < end7 : j > end7; asc7 ? j++ : j--) {
          var errorX = x + j - matrixWidthAdj;
          var errorY = y + i; // Bounds checking - don't want to write past the end of the array
  ...
  for (y = 0, end8 = height, asc8 = 0 <= end8; asc8 ? y < end8 : y > end8; asc8 ? y++ : y--) {
    ...
    for (x = 0, end9 = width, asc9 = 0 <= end9; asc9 ? x < end9 : x > end9; asc9 ? x++ : x--) {
      var isBlack = output[x][y] < 128;
      var value = isBlack ? 0 : 255;
